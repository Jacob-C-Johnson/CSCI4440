...
var rotatebase = false;
var rotatelower = false;
var rotateupper = false;
var upperdirection = 1;
var lowerdirection = 1;
...
window.onload = function init() {
    ...
    document.getElementById("lower").onclick = function(){rotatelower = !rotatelower;};

    document.getElementById("upper").onclick = function(){rotateupper = !rotateupper;};
...
var render = function() {
    ...
    if (rotatebase) theta[Base] += 0.5;

    if (rotatelower) {
        theta[LowerArm] += 0.5 * upperdirection;
        // keep theta alternating bwtween -75 and 75
        if (theta[LowerArm] > 75 || theta[LowerArm] < -75) upperdirection = upperdirection * -1;
    }

    if (rotateupper) {
        theta[UpperArm] += 0.5 * lowerdirection;
        // keep theta alternating bwtween -75 and 75
        if (theta[UpperArm] > 75 || theta[UpperArm] < -75) lowerdirection = lowerdirection * -1;
    }
